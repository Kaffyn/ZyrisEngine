...
- [ ] **Save Server** -  Orquestrador de Persistência e Serialização
  - **Threaded Architecture** - Operações de I/O e criptografia ocorrem em threads dedicadas, garantindo salvamento sem *stuttering* (frames congelados).
  - **Smart Reference Handling** - Serializador inteligente que rastreia UIDs para resolver referências cíclicas e sub-recursos compartilhados, evitando duplicação de dados.
  - **Protocolo de Nó:**
    - `_save_state() -> Dictionary`: Método virtual onde o objeto define sua própria serialização.
    - `_load_state(data)`: Chamado automaticamente pelo servidor ao restaurar instâncias.
  - **Integração com LSS (State Injection):**
    - **Snapshot** - Congelamento de estado via servidor solicitado pelo LSS antes de descarregar zonas.
    - **Hydration** - Injeção de dados injetados pelo servidor antes do nó entrar na SceneTree.
  - **Modos de Operação** - Suporte nativo para Slots Manuais, Checkpoints e Quick-Save.
