...
- **Protocolo Declarativo (@persistent)** - Automação total via anotações no GDScript e flags no C++. Basta marcar uma variável como `@persistent` para que ela seja incluída no sistema de salvamento. Suporta `@persistent_group` para organização por tags.
...
- [ ] **Inventory System** - Gestão de Items e Transações
  ...
  - **Equipment Bridge (GAS)** - Ao equipar um item, o sistema injeta/remove automaticamente _Gameplay Abilities_ e _Effects_ no componente do portador, sem scripts manuais.
...
- [ ] **Sounds Engine** — Audio Middleware & Design
    ...
    **Filosofia do Sistema:**
    Ao invés do programador escolher _qual_ arquivo tocar, ele apenas sinaliza _o que_ aconteceu (ex: `PlayEvent("Passos")`). O Sounds Engine decide o resultado sonoro baseado em lógica pré-configurada pelo Sound Designer.
