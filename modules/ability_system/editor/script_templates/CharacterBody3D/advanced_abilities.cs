...
public partial class _CLASS_ : _BASE_
{
    ...
    private void SetupPersistenceId()
    {
        ...
        {
            PersistenceId = $"character_3d_{GetInstanceId()}";
        }
    }
    ...
    public override void _SavePersistence(Godot.Collections.Dictionary state)
    {
        ...
        if (state.ContainsKey("movement"))
        {
            var movementData = (Godot.Collections.Dictionary)state["movement"];
            movementData["camera_rotation"] = new Godot.Collections.Dictionary
            {
                {"x", _cameraRotation.X},
                {"y", _cameraRotation.Y}
            };
    ...
    public override void _LoadPersistence(Godot.Collections.Dictionary data)
    {
        ...
        {
            var movementData = (Godot.Collections.Dictionary)data["movement"];
            if (movementData.ContainsKey("camera_rotation"))
            {
                var camData = (Godot.Collections.Dictionary)movementData["camera_rotation"];
                _cameraRotation.X = camData.Get("x", 0.0f).AsSingle();
                _cameraRotation.Y = camData.Get("y", 0.0f).AsSingle();

                if (_camera3D != null)
                {
                    _camera3D.Rotation.X = _cameraRotation.X;
                    Rotation.Y = _cameraRotation.Y;
                }
            }
        }
    }
