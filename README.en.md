...
- [x] **Gameplay Ability System (GAS)** - Data-Oriented Gameplay Framework
  ...
  - **Attribute Events**: Signals for attribute changes (ideal for UI and triggers).
    ...
    **Editor Integration (GAS Studio):**
    **GAS Studio** is a complete modal editor integrated into the Inspector:
  - **Floating Graph Window** - Quick visual editing without losing scene context.
  - **Tag Management** - Hierarchical `GameplayTags` visual selector.
  - **Live Debug** - Real-time visualization of attribute and effect flows.

    **Differentials:**
  - **Native Multiplayer** - Client prediction and server reconciliation (Robust Netcode).
